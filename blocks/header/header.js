...
async function buildBreadcrumbsFromNavTree(nav, currentUrl) {
  const crumbs = [];
  console.log(nav);
  const homeUrl = document.querySelector('.default-content-wrapper a').href;
...
async function buildBreadcrumbs() {
  ...
  const crumbs = await buildBreadcrumbsFromNavTree(document.querySelector('.siteNav'), document.location.href);
...
export default async function decorate(block) {
  ...
  // decorate nav DOM
  block.textContent = '';
  // loadSiteHeader(fragment)
  const nav = document.createElement('div');
  nav.dir = 'ltr';
  while (fragment.firstElementChild) nav.append(fragment.firstElementChild);
  nav.querySelector('.default-content-wrapper').remove();
  //
